...
export async function initVocabPage() {
  ...
  if (CURRENT_THEME.source === 'firestore') {
     data = typeof CURRENT_THEME.vocabData === 'string' 
    ...
    if (!window.VOCABS) {
        ...
        } catch(e) { console.warn("Failed to load local vocabs", e); }
    }
    
    if (window.VOCABS) {
        data = window.VOCABS.filter(v => v.tema === CURRENT_THEME.vocabFilter || v.tema === THEME_ID);
    }
  }
    ...
    slice.forEach((item, i) => {
      ...
      tableBody.innerHTML += `
        <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
          <td class="px-6 py-4 text-sm text-slate-500 dark:text-slate-400 font-bold">${start + i + 1}</td>
          <!-- JAMAK (Column 2) -->
     ...
     slice.forEach((item, i) => {
        ...
        <div class="bg-white dark:bg-slate-800 rounded-[1.5rem] p-6 border border-slate-50 dark:border-slate-700 shadow-[0_2px_20px_rgba(0,0,0,0.03)] mb-4 relative hover:shadow-lg transition-shadow duration-300">
           ...
           <div class="flex justify-between items-start mb-6 w-full">
              <!-- Rank (Top Left) -->
              <span class="text-sm font-bold text-slate-300 dark:text-slate-600 font-sans">#${i + 1}</span>
...
function updatePagination(page, totalPages, renderFn) {
    ...
    if(page > 1) {
       ...
       btn.textContent = "←";
       btn.className = "px-3 py-1 rounded bg-slate-200 hover:bg-slate-300 dark:bg-slate-700 dark:text-white";
...
export async function initReadingPage() {
  ...
  try {
    let data = [];
    if(CURRENT_THEME.source === 'firestore') {
         data = typeof CURRENT_THEME.readData === 'string' 
    ...
    function showReading(idx) {
       ...
       if(typeof item === 'string') {
           ...
           idnText = item.idn || item.arti || item.mean || item.trans || "(Terjemahan tidak tersedia)";
           audioSrc = item.audio || "";
       }
    ...
    console.error(e);
    if(readingTextEl) readingTextEl.innerHTML = "❌ Gagal memuat modul bacaan.";
  }
}
...
export async function initQuizPage() {
  ...
  try {
     let QUESTIONS = [];
     if(CURRENT_THEME.source === 'firestore') {
         QUESTIONS = typeof CURRENT_THEME.quizData === 'string' 
     ...
     if(!QUESTIONS || QUESTIONS.length === 0) {
        if(questionTextEl) questionTextEl.innerHTML = "❌ Data kuis kosong / belum dibuat.";
        return;
     }
     ...
     window.checkAnswer = (val, correct, btn) => {
        ...
        if(val === correct) {
           ...
           btn.classList.add('bg-green-100', 'border-green-500', 'text-green-700', 'dark:bg-green-900/50', 'dark:text-green-300', 'dark:border-green-500');
           btn.classList.remove('bg-slate-50', 'border-slate-200');
           if(fbEntry) fbEntry.innerHTML = "<span class='text-green-600 dark:text-green-400 font-bold text-lg'>✅ Mumtaz! Benar!</span>";
        } else {
           // SALAH
           btn.classList.add('bg-red-100', 'border-red-500', 'text-red-700', 'dark:bg-red-900/50', 'dark:text-red-300', 'dark:border-red-500');
           btn.classList.remove('bg-slate-50', 'border-slate-200');
           if(fbEntry) fbEntry.innerHTML = `<div class='text-red-600 dark:text-red-400 font-bold'>❌ Kurang tepat.</div><div class='text-sm text-slate-500'>Jawaban: ${correct}</div>`;
           
           // Highlight correct one so they learn
           allBtns.forEach(b => {
             if(b.innerText.trim() === correct) {
                 b.classList.add('bg-green-50', 'border-green-400', 'text-green-700', 'dark:bg-green-900/30', 'dark:text-green-300');
             }
           });
        }
    ...
    console.error(e);
    if(questionTextEl) questionTextEl.innerHTML = "❌ Gagal memuat modul kuis.";
  }
}
